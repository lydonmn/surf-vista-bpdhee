...
export default function HomeScreen() {
  ...
  console.log('[HomeScreen iOS] Theme loaded:', !!theme);
  
  const { user, profile, checkSubscription, isLoading, isInitialized, refreshProfile } = useAuth();
  console.log('[HomeScreen iOS] Auth state - user:', !!user, 'profile:', !!profile, 'isLoading:', isLoading);
  
  const isSubscribed = checkSubscription();
  console.log('[HomeScreen iOS] Subscription status:', isSubscribed);
  
  const { currentLocation, locationData } = useLocation();
  console.log('[HomeScreen iOS] Location - current:', currentLocation, 'data:', !!locationData);
  
  const { surfReports, surfConditions, weatherData, weatherForecast, isLoading: surfLoading, error, refreshData } = useSurfData();
  console.log('[HomeScreen iOS] Surf data - reports:', surfReports?.length, 'conditions:', !!surfConditions, 'loading:', surfLoading);
  ...
  const todayDate = useMemo(() => {
    ...
    console.log('[HomeScreen iOS] Today\'s date:', date);
    return date;
  }, []);
  ...
  if (!user || !isSubscribed) {
    console.log('[HomeScreen iOS] Showing locked content - user:', !!user, 'subscribed:', isSubscribed);
